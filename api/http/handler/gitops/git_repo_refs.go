...
// @summary List references (branches and tags) from a git repository
// @description List all git references from a repository
...
func (handler *Handler) gitOperationRepoRefs(w http.ResponseWriter, r *http.Request) *httperror.HandlerError {
	...
	if err != nil {
		...
		if errors.Is(err, gittypes.ErrIncorrectRepositoryURL) {
			return httperror.BadRequest("Invalid repository URL", err)
		}
		return httperror.InternalServerError("Unable to list repository references", err)
	}
